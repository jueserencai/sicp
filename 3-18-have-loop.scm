...
设置一个唯一的标识符 identity (可以用 cons 配合 eq? 来做到这一点)
遍历列表，使用 eq? 检查列表的每个序对的 car 部分是否和 identity 相等，如果相等的话，那么这个列表有环，如果不相等，那么将这个序对的 car 部分设置为 identity ，然后继续遍历列表的 cdr 部分，直到发现环或者列表为空为止。
以下是过程的定义：
